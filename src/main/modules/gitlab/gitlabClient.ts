...
const getClient = (hostUrl: string, token: string) => {
...
export const fetchOpenMrs = async (
  ...
  token: string,
  projectId: string,
): Promise<Result<GitLabMergeRequest[], string>> => {
  try {
    if (!token) {
    ...
    const mrs: GitLabMergeRequest[] = await Promise.all(
      rawList.map(async (item: any) => {
        const mr = mapMr(item);
        if (mr.pipelineStatus === "none") {
          try {
            const pipeRes = await client.get(
              `/projects/${encodeURIComponent(projectId)}/merge_requests/${item.iid}/pipelines`,
            );
...
const mapUser = (u: any): GitLabUser => ({
  name: u?.name || "Desconocido",
  username: u?.username || "unknown",
  avatarUrl: u?.avatar_url || "",
});

const mapMr = (item: any): GitLabMergeRequest => ({
  id: item.id,
  iid: item.iid,
  ...
  labels: Array.isArray(item.labels)
    ? item.labels.map((l: any) => (typeof l === "string" ? l : l.name || String(l)))
    : [],
  pipelineStatus:
    item.head_pipeline?.status || item.pipeline?.status || item.pipeline_status || "none",
...
export const fetchMrDetails = async (
  ...
  projectId: string,
  mrIid: number,
  mrTitleFallback?: string,
): Promise<Result<GitLabMrDetails, string>> => {
  try {
    if (!token) {
      // Mock details
      const isCompliant = mrIid !== 3;
      const commits: GitLabCommit[] = isCompliant
        ? [
            {
              sha: "a1b2c3d4e5",
              shortSha: "a1b2c3d",
    ...
    const mr: GitLabMergeRequest = mapMr(item);
    if (mr.pipelineStatus === "none") {
      try {
        const pipeRes = await client.get(
    ...
    const commits: GitLabCommit[] = (commitsRes.data || []).map((c: any) => ({
      sha: c.id,
      shortSha: c.short_id,
    ...
    const diffs: GitLabDiff[] = (changesRes.data?.changes || []).map((d: any) => ({
      oldPath: d.old_path,
      newPath: d.new_path,
      diff: d.diff,
    ...
    return err(
      `Error obteniendo detalles del MR #${mrIid}: ${e.response?.data?.message || e.message}`,
    );
  }
};
...
export const approveMr = async (
  ...
  try {
    if (!token) return ok(true); // Mock success
    const client = getClient(hostUrl, token);
...
export const mergeMr = async (
  ...
  try {
    ...
    return err(
      `Error al realizar Merge en el MR #${mrIid}: ${e.response?.data?.message || e.message}`,
    );
  }
};
